...
part 'character_card_parts/character_card_presentation.dart';
...
class _CharacterCardState extends State<CharacterCard>
  ...
  static const Duration _textDraftSyncDelay = Duration(milliseconds: 450);
  ...
  CharacterCardData? _pendingTextDraft;
  bool? _isEditing;
  ...
  bool _hasPendingTextChanges(CharacterCardData data) {
    return data.quote != widget.data.quote ||
        data.motto != widget.data.motto ||
        data.synopsis != widget.data.synopsis;
  }

  void _clearPendingTextSync() {
    _textDraftSyncTimer?.cancel();
    _textDraftSyncTimer = null;
    _pendingTextDraft = null;
  }

  void _emitEditedData(CharacterCardData data) {
    _clearPendingTextSync();
    widget.onEdited?.call(data);
  }

  void _queueTextDraftSync() {
    final next = _buildCurrentEditedData();
    if (!_hasPendingTextChanges(next)) {
      _clearPendingTextSync();
      return;
    }

    _pendingTextDraft = next;
    _textDraftSyncTimer?.cancel();
    _textDraftSyncTimer = Timer(_textDraftSyncDelay, _flushPendingTextSync);
  }

  void _flushPendingTextSync() {
    final next = _pendingTextDraft;
    _clearPendingTextSync();
    if (next == null || !_hasPendingTextChanges(next)) {
      return;
    }
  ...
  Future<void> _selectBirthday() async {
    ...
    _emitEditedData(_buildCurrentEditedData());
  }
